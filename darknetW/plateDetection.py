...
class PlateDetect:
    ...
    video_writer.open('cutout' + str(time.time()) + '.avi', fourcc, 20.0, (int(width), int(height)),
                      True)
    ...
    def print(self):
        ...
        for i in range(len(boxes)):
            if i in indexes:
                ...
                cv2.imshow("Image" + str(time.time()), frameR)
                # video_writer.write(resized)
                # cv2.imwrite("imageBre" + str(time.time()) + ".jpg", resized)

    def startPlateDetection(self):
